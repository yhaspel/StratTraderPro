...
- Grafana Explore: `sum by (result) (rate(auth_login_total{result="locked"}[5m]))`. (The Auth Health dashboard was retired by ADR-109; it never carried a lockout panel.)
