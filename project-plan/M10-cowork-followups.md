...
for StratTraderPro. Repo: /Users/yuval3000/Documents/Claude/Projects/StratTraderPro. Work through
...
on the backend + grafana-agent services; AUDIT_ALERT_EMAIL=yuval3000@gmail.com; TASK_METRICS_PORT
...
auth and the agent scrapes it with matching creds. Prod backend: https://backend-production-f3e8.up.railway.app ;
staging: https://backend-staging-4b6d.up.railway.app . Grafana Cloud stack: account yuval3000
(Prometheus datasource `grafanacloud-yuval3000-prom`). Both environments are currently healthy
...
## SECTION B — Grafana Cloud (the yuval3000 stack, e.g. https://yuval3000.grafana.net)
B1. Import the six dashboards (Dashboards → New → Import → upload JSON), from `infra/grafana/`:
    ...
    `grafanacloud-yuval3000-prom` datasource. Verify the new "SLO" + "Last incident" panels render.
...
B3. Contact points (Alerting → Contact points): (1) email → yuval3000@gmail.com. (2) Telegram —
