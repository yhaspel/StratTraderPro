...
| 7 | Teardown — delete the `adr109-cutover` service account, then `rm .env.grafana` | ✅ **DONE 2026-09-21** (48 days late — the no-expiry Admin token sat live the whole time). Service account id `19` deleted (`HTTP 200`), revocation confirmed by the token returning **401** on `/api/folders` and `/api/v1/provisioning/alert-rules`; `.env.grafana` removed. The `adr109-h3-soak-verdict` registry entry was already gone, so the 2027-08-04 re-fire risk is closed; its folder remains on disk but is inert without a registry entry (kept for `h3-check.py`). `.env.grafana.local` is a **different**, still-required credential (the agent's `glc_` MetricsPublisher token) and was left alone. |
