...
| AC-10-9 | **[CI]** Alert rules covering the reconciled master-plan §9 table (§6.5, incl. the three M09 backtest rules) are committed as Grafana-provisioning files, and a unit test cross-checks every referenced metric name against the codebase's exported metric names. **[LIVE]** Rules imported to Grafana Cloud; a sample alert fires and is received on both email and Telegram contact points. |
