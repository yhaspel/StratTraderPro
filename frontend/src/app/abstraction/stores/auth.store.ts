...
// P1-4: legacy key — the refresh token used to be persisted here (XSS-readable).
// It now lives in an HttpOnly cookie the SPA can't read; purge any stale value.
const LEGACY_REFRESH_KEY = 'stp_refresh_token';
...
export class AuthStore {
  ...
  constructor() {
    // One-time migration: drop the pre-P1-4 refresh token from localStorage.
  ...
  setAuthed(user: AuthUser, access: string): void {
    this._user.set(user);
  ...
  updateTokens(access: string): void {
    this._accessToken.set(access);
  }
}
