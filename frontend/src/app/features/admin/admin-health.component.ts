    ...
    <div class="mx-auto max-w-6xl space-y-6 p-6">
      ...
      @if (!admin.loading() && admin.health(); as h) {
        ...
        <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <app-card>
            <div class="text-[10px] uppercase tracking-[.1em] text-neutral-600">{{ 'admin.health.hmm_age' | translate }}</div>
        ...
        <app-card>
          ...
          @if (isEmpty(h.queue_depths)) {
            <p class="text-sm text-neutral-600">{{ 'admin.health.no_queues' | translate }}</p>
          } @else {
        ...
        <app-card>
          ...
          @if (isEmpty(h.broker_streams)) {
            <p class="text-sm text-neutral-600">{{ 'admin.health.no_streams' | translate }}</p>
          } @else {
            <ul class="divide-y divide-divider text-[13px]">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <app-card>
            ...
            @if (h.active_halts.length === 0) {
              <p class="text-sm text-neutral-600">{{ 'admin.health.no_halts' | translate }}</p>
            } @else {
          ...
          <app-card>
            ...
            @if (h.flags_overridden.length === 0) {
              <p class="text-sm text-neutral-600">{{ 'admin.health.no_overrides' | translate }}</p>
            } @else {
        ...
        <p class="text-xs text-neutral-600">{{ 'admin.health.generated' | translate }}: {{ h.generated_at | date:'medium' }}</p>
      }
    </div>
  `,
})
